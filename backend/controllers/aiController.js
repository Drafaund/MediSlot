...
const symptomCheck = async (req, res) => {
  try {
    ...
    if (!symptoms || !symptoms.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Gejala wajib diisi" });
    }
  ...
  try {
    ...
    if (records.length === 0) {
      return res.json({
        ...
        data: {
          overview:
            "No medical history found. Your health summary will be available once you have medical records from a doctor visit.",
          chronicConditions: [],
          ongoingMedications: [],
          patterns: [],
          recommendations: [],
    ...
    const recordsText = records
      .map((r, idx) => {
        ...
        const complaint =
          r.chiefComplaint || r.symptoms.join(", ") || "Not recorded";
        const drugs =
        ...
        const vitalParts = [];
        if (r.vitalSigns?.bloodPressure)
          vitalParts.push(`BP: ${r.vitalSigns.bloodPressure}`);
        if (r.vitalSigns?.heartRate)
          vitalParts.push(`HR: ${r.vitalSigns.heartRate} bpm`);
        if (r.vitalSigns?.temperature)
          vitalParts.push(`Temp: ${r.vitalSigns.temperature}°C`);
        if (r.vitalSigns?.weight)
          vitalParts.push(`Weight: ${r.vitalSigns.weight} kg`);
        const vitals =
          vitalParts.length > 0 ? vitalParts.join(", ") : "Not recorded";
...
- Avoid overly technical medical terminology`,
    ...
    res.json({
      ...
      data: {
        ...
        chronicConditions:
          parsed.monitoredConditions || parsed.chronicConditions || [],
