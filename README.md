...
> Capstone project · MongoDB Developer Path · Universitas Gadjah Mada · 2026

**Live Demo:**

- Frontend: https://medislot-frontend-phi.vercel.app
- Backend API: https://medislot-production-53f9.up.railway.app
...
- In-app notifications for booking confirmations, cancellations, and record creation
...
- In-app notifications for new bookings and appointment changes
...
│   ├── utils/notify.js                  ← helper to create Notification documents
│   ├── .env.example
│   └── server.js
└── frontend/
    └── src/
        ├── components/
        │   ├── ui.jsx                   ← shared design system (Card, Badge, Btn, …)
        │   ├── Navbar.jsx
        │   ├── DoctorCard.jsx
        │   ├── Sidebar.jsx
        │   └── TopBar.jsx
        ...
        │   ├── GoogleSuccess.jsx        ← handles ?token= from OAuth redirect
...
PUT    /api/doctors/:id/verify              [JWT, admin]
```
...
GET    /api/medical-records/appointment/:appointmentId    [JWT]
...
acceptBPJS, bio, isVerified, verificationStatus(pending|verified|rejected),
...
```
_id, userId(→User), type(booking_new|appointment_confirmed|
appointment_cancelled|appointment_cancelled_patient|
appointment_cancelled_doctor|appointment_completed|
record_created|doctor_registered|doctor_verified|doctor_rejected),
title, message, isRead(bool), relatedId(ObjectId|null), timestamps
```
...
- Notifications are in-app only (no email/push); auto-created on booking, status changes, record creation, and doctor verification results
...
| [Railway](https://railway.app)                | Backend (Node/Express) |
| [MongoDB Atlas M0](https://cloud.mongodb.com) | Database (free tier)   |

Set `REACT_APP_API_URL=https://medislot-production-53f9.up.railway.app/api` in Vercel environment variables.  
